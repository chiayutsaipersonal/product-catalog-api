...
function pagination (getRecordCountFn = () => Promise.resolve(0)) {
  return async (req, res, next) => {
    ...
    let linkHeaderProps = ['first', 'prev', 'self', 'next', 'last']
    linkHeaderProps.forEach(linkHeaderProp => {
      if (linkHeaderProp in linkHeader) {
        for (let urlQueryProp in query) {
          if (urlQueryProp !== 'page' && urlQueryProp !== 'per_page') {
            ...
            if (query[urlQueryProp]) {
              linkHeader[linkHeaderProp].url += `=${query[urlQueryProp]}`
            }
          }
        }
      }
...
function appendPaginationData (req, res, next) {
  if ('linkHeader' in req) {
    let reqLinkHeader = req.linkHeader
    Object.assign(req.resJson, {
      pagination: {
        totalRecords: reqLinkHeader.last.per_page * reqLinkHeader.last.page,
        totalPages: reqLinkHeader.last.page,
        perPage: reqLinkHeader.last.per_page,
        currentPage: reqLinkHeader.self.page,
        first: reqLinkHeader.first.url,
        prev:
          reqLinkHeader.prev === undefined ? undefined : reqLinkHeader.prev.url,
        self: reqLinkHeader.self.url,
        next:
          reqLinkHeader.next === undefined ? undefined : reqLinkHeader.next.url,
        last: reqLinkHeader.last.url,
      },
    })
  }
}
